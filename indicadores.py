...
# Embedded file name: C:\Users\Developer\proijects\bot-python\indicadores.py
# Compiled at: 2020-05-03 12:51:30
# Size of source mod 2**32: 15828 bytes
import numpy, pandas as pd, math as m, numpy as np

def bollingerBands(price_series, window_size, num_of_std):
    stock_price = pd.DataFrame(price_series)
...
def EMA(df, n):
    EMA = pd.Series(pd.ewma((np.array(df['close'])), span=n, min_periods=(n - 1)), name=('EMA_' + str(n)))
    df = df.join(EMA)
    return df
...
def BBANDS(df, n):
    MA = pd.Series(df.rolling_mean(df['close'], n))
    MSD = pd.Series(df.rolling_std(df['close'], n))
    b1 = 4 * MSD / MA
